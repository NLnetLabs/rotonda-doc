...
Besides using our new RIB type, we’ve changed the input type to that RIB to ``(status, as_path)``, and therefore it also outputs that same type. The user can query the application, and will also get instances of that same type out.
...
We have now called the RIB we already had, `pRIB’ as an abbreviation for `physical RIB` to set it apart from the virtual RIB, ‘vRIB’.
Our vRIB here is dependent on the pRIB from which routes flow come flowing in. It adds additional filtering and transforms the incoming routes, while being fully queryable, like the physical RIB.
